<template>
    <div class="max-w-3xl mx-auto">
        ...
        <div class="text-center mb-10">
            <h2
                class="text-3xl sm:text-4xl font-bold text-[#10B981] mb-4 tracking-tight"
            >
        ...
        >
            <div class="flex items-center gap-3">
                ...
                <div class="text-green-800 font-medium text-sm sm:text-base">
                    ແບບຈຳລອງ AI
                    ໄດ້ປ່ຽນສຽງເວົ້າເປັນຂໍ້ຄວາມສຳເລັດແລ້ວດ້ວຍລະດັບຄວາມເຊື່ອໝັ້ນ
                    {{ confidence || 0 }}%
                </div>
            </div>
        </div>
        ...
        <div
            class="bg-white/70 backdrop-blur-lg rounded-xl border border-slate-200/60 shadow-lg overflow-hidden mb-8"
        >
            ...
            <div class="h-72 sm:h-80 overflow-y-auto p-6">
                <div
                    class="text-slate-900 text-base leading-relaxed whitespace-pre-wrap cursor-text select-text"
                >
                    {{
                        transcribedText ||
                        "No transcription text available. Check console for debugging info."
                    }}
                </div>
                <!-- DEBUG INFO -->
        ...
        >
            ...
            >
                <Icon
                    :name="copySuccess ? 'mdi:check' : 'mdi:content-copy'"
                    class="w-5 h-5"
                />
                <span>{{
                    copySuccess ? "Copied!" : transcriptionContent.copyButton
                }}</span>
            </button>
        ...
        <!-- Start Over Section -->
        <div
            class="bg-white/60 backdrop-blur-lg rounded-xl border border-slate-200/60 p-6 text-center"
        >
...
const totalDuration = computed(() => {
    ...
    if (transcriptionStore.uploadedFile) {
        return (
            transcriptionStore.duration || transcriptionStore.recordedTime || 0
        );
    }
...
const downloadTranscription = () => {
    ...
    const blob = new Blob([transcribedText.value], {
...
onMounted(() => {
    ...
    if (
        !audioUrl.value &&
        (transcriptionStore.audioBlob || transcriptionStore.uploadedFile)
    ) {
